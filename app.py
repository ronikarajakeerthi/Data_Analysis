...
st.set_page_config(
    page_title="Online Retail Customer Prediction",
    page_icon="🛒",
    layout="wide"
)
...
st.title("🛒 Online Retail Customer Prediction")
st.caption(
    "K-Means customer segmentation based on spending, quantity purchased, "
    "and number of orders."
)
...
# Find the CSV file in the same folder as app.py.
base_folder = os.path.dirname(os.path.abspath(__file__))
file_path = os.path.join(base_folder, "Online_Retail.csv")
...
try:
    ...
    st.error(
        "Online_Retail.csv was not found. "
        "Please make sure Online_Retail.csv is in the same GitHub folder as app.py."
    )
    st.stop()
...
# Keep only positive sales records.
df = df[df["Quantity"] > 0]
...
if filtered_df.empty:
    st.error("No customer data is available for the selected country.")
    st.stop()


# ============================================================
# 8. CREATE CUSTOMER DATA
# ============================================================
...
# ============================================================
# 9. CHECK CUSTOMER COUNT
# ============================================================

if len(customer_data) < number_of_clusters:
    st.warning(
        f"Only {len(customer_data)} customers are available "
        f"for the selected country. Please choose "
        f"{len(customer_data)} or fewer customer groups."
    )
    st.stop()
...
# These are the three features used for segmentation.
features = [
    "TotalSpending",
    "TotalQuantity",
    "NumberOfOrders"
...
# Scale the features so that large values do not dominate.
scaler = StandardScaler()
...
customer_data["Cluster"] = model.fit_predict(X_scaled)


# ============================================================
# 11. CREATE PROFESSIONAL CUSTOMER SEGMENTS
# ============================================================
...
# Scale the cluster averages to calculate a simple value score.
cluster_features = [
    "AverageSpending",
    "AverageQuantity",
    "AverageOrders"
...
# Sort clusters from highest to lowest customer value.
sorted_clusters = (
    cluster_summary
    .sort_values(
        "ValueScore",
...
# Give easy-to-understand names to the clusters.
segment_names = {}

if number_of_clusters == 2:

    segment_names[sorted_clusters[0]] = "High-Value Customer"
    segment_names[sorted_clusters[1]] = "Low-Value Customer"

elif number_of_clusters == 3:

    segment_names[sorted_clusters[0]] = "High-Value Customer"
    segment_names[sorted_clusters[1]] = "Regular Customer"
    segment_names[sorted_clusters[2]] = "Low-Value Customer"
    ...
    for position, cluster in enumerate(sorted_clusters):
        ...
        if position == 0:
            segment_names[cluster] = "High-Value Customer"

        elif position == len(sorted_clusters) - 1:
            segment_names[cluster] = "Low-Value Customer"

        else:
            segment_names[cluster] = f"Regular Customer {position}"
...
st.sidebar.caption(
    "Enter the customer's details to get an automatic prediction."
)

# User enters spending in millions.
...
# Convert the entered million value back to the original value.
# K-Means was trained using the original spending values.
manual_spending = (
    manual_spending_millions * 1_000_000
)


# ============================================================
# 13. PREDICT CUSTOMER GROUP
# ============================================================

manual_customer = pd.DataFrame(
    {
        "TotalSpending": [manual_spending],
        "TotalQuantity": [manual_quantity],
        "NumberOfOrders": [manual_orders]
    }
)

# Scale the new customer's values using the same scaler.
manual_scaled = scaler.transform(
    manual_customer[features]
)
...
# Convert the cluster number into a professional segment name.
...
# ============================================================
# 14. DISPLAY ONLY THE PREDICTED ANSWER
# ============================================================
...
st.write(
    f"Based on the entered customer details, the K-Means model "
    f"predicts that this customer belongs to the "
    f"**{predicted_segment}** group."
)

st.caption(
    "Prediction is based on total spending, total quantity purchased, "
    "and number of orders."
)
